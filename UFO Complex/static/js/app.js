...
function handleClick() { // Define a function called handleClick() that will reference to the button function
 ...
 // Use d3.select() to select first element, if none given empty 
  ...
  else if (city) { // Check to see if a date was entered and filter the data using that date.
    searchData = searchData.filter(row => row.city === city); // Apply `filter` to the table data to only keep the rows where the `datetime` value matches the filter value
  }
  else if (state) { // Check to see if a date was entered and filter the data using that date.
    searchData = searchData.filter(row => row.state === state); // Apply `filter` to the table data to only keep the rows where the `datetime` value matches the filter value
  }
  else if (country) { // Check to see if a date was entered and filter the data using that date.
    searchData = searchData.filter(row => row.country === country); // Apply `filter` to the table data to only keep the rows where the `datetime` value matches the filter value
  }
  else if (shape) { // Check to see if a date was entered and filter the data using that date.
    searchData = searchData.filter(row => row.shape === shape); // Apply `filter` to the table data to only keep the rows where the `datetime` value matches the filter value
  }
